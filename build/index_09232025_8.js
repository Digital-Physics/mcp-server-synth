...
const DRUM_PATTERNS = {
    ...
    verse: (track) => {
        track.addEvent([
            ...
            new MidiWriter.NoteEvent({ pitch: ['F#1'], duration: '4', channel: 10, velocity: 70 }), // Closed Hat
        ], (event, index) => ({ startTick: index * 128 }));
    },
    chorus: (track) => {
        track.addEvent([
...
function generateArpeggioTrack(structure, chords, instrument) {
    ...
    structure.forEach(section => {
        if (section.name !== 'chorus')
            return; // Only play arpeggio in chorus for impact
        ...
        for (let i = 0; i < section.length; i++) {
            ...
            for (let j = 0; j < 4; j++)
                pattern(track, chord); // Repeat pattern for the whole measure
        }
    });
    return track;
}
...
// --- MCP Tool Definition ---
server.tool("play_retrowave_song_pro", "Generates and plays a complex, layered retro-ambient EDM synth song with dynamic structure.", { tone: z.string().optional().describe("Mood or tone (aesthetic only).") }, async ({ tone }) => {
    try {
