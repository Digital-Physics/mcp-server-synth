...
function generateDrumTrack(length) {
    ...
    for (let i = 0; i < length; i++) {
        if (Math.random() > 0.3)
            track.addEvent(new MidiWriter.NoteEvent({ pitch: [drums[0]], duration: "4" })); // kick
        if (Math.random() > 0.6)
            track.addEvent(new MidiWriter.NoteEvent({ pitch: [drums[1]], duration: "4" })); // snare
        if (Math.random() > 0.5)
            track.addEvent(new MidiWriter.NoteEvent({ pitch: [drums[2]], duration: "8" })); // hi-hat
    }
    return track;
}
