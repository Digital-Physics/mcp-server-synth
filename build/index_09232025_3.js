...
function addDelay(buffer, delaySamples, feedback = 0.3) {
...
function distortBuffer(buffer, gain = 2.0) {
...
// --- Master FX Chain ---
function masterFX(buffer) {
    let out = buffer;
    if (Math.random() > 0.5)
        out = addDelay(out, 2000, 0.25);
    if (Math.random() > 0.5)
        out = distortBuffer(out, 1.5 + Math.random());
    return out;
}
// --- Mixing ---
...
function getSamplesFromFolder(folderName) {
...
// --- Composition Logic ---
function getComposition(tone, structure) {
    const allSamples = {
...
// --- Generate WAV track ---
async function generateLayeredTrackFile(tone) {
    ...
    for (const track of tracks) {
        for (const event of track.events) {
            ...
            if (Math.random() > 0.8)
                sampleBuffer = pitchShift(sampleBuffer, Math.floor(Math.random() * 7 - 3));
            if (Math.random() > 0.9)
                sampleBuffer = reverseBuffer(sampleBuffer);
            if (Math.random() > 0.7)
                sampleBuffer = distortBuffer(sampleBuffer, 1.0 + Math.random() * 2);
...
// --- MCP Tool ---
server.tool("play_layered_music", "Play music with multiple layered tracks, clever sampling, FX, and structure.", {
