...
const DRUM_PATTERNS = {
    ...
    verse: (track) => { track.addEvent([new MidiWriter.NoteEvent({ pitch: ['C1'], duration: '4', channel: 10, velocity: 100 }), new MidiWriter.NoteEvent({ pitch: ['F#1'], duration: '4', channel: 10, velocity: 70 }), new MidiWriter.NoteEvent({ pitch: ['D1'], duration: '4', channel: 10, velocity: 90 }), new MidiWriter.NoteEvent({ pitch: ['F#1'], duration: '4', channel: 10, velocity: 70 }),], (event, index) => ({ startTick: index * 128 })); },
    chorus: (track) => { track.addEvent([new MidiWriter.NoteEvent({ pitch: ['C1'], duration: '4', channel: 10, velocity: 110 }), new MidiWriter.NoteEvent({ pitch: ['G#1'], duration: '8', channel: 10, velocity: 80 }), new MidiWriter.NoteEvent({ pitch: ['D1'], duration: '4', channel: 10, velocity: 100 }), new MidiWriter.NoteEvent({ pitch: ['G#1'], duration: '8', channel: 10, velocity: 80 }),], (event, index) => ({ startTick: index * 128 })); },
...
function generateSongMIDI() {
    ...
    const description = `Structure: ${structure.map(s => s.name).join(' -> ')}. Scale: ${Object.keys(SCALES).find(k => SCALES[k] === scale)}.`;
...
// --- MCP Tool Definition ---
server.tool("play_ambient_retrowave_song", "Generates and plays a complex, layered retro-ambient EDM synth song with a looping nature soundscape.", { tone: z.string().optional().describe("Mood or tone (aesthetic only).") }, async ({ tone }) => {
    try {
