...
function generateRichNoteBuffer(freq: number, noteLength: number, sampleRate: number): Float32Array {
  ...
  for (let i = 0; i < totalSamples; i++) {
    ...
    else if (t < attackTime + decayTime) env = 1 - (1 - sustainLevel) * ((t - attackTime) / decayTime);
    else if (t > noteLength - releaseTime) env *= (noteLength - t) / releaseTime;
