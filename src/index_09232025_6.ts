...
function randomScale() {
  const keys = Object.keys(ambientScales) as (keyof typeof ambientScales)[];
...
function generateMelodyTrack(length: number) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    const pitches = [];
    for (let j = 0; j < chordSize; j++) pitches.push(randomChoice(scale));
...
// --- Pad Track ---
function generatePadTrack(length: number) {
  const track = new MidiWriter.Track();
...
function generateDrumTrack(length: number) {
  ...
  for (let i = 0; i < length; i++) {
    if (Math.random() > 0.5) track.addEvent(new MidiWriter.NoteEvent({ pitch: [kick], duration: "4", channel: 10 }));
    if (Math.random() > 0.7) track.addEvent(new MidiWriter.NoteEvent({ pitch: [snare], duration: "4", channel: 10 }));
    if (Math.random() > 0.3) track.addEvent(new MidiWriter.NoteEvent({ pitch: [hihat], duration: randomChoice(["8", "16"]), channel: 10 }));
  }
...
server.tool(
  "play_synth_song",
  "Generate and play a layered ambient synth song.",
