...
function generateDrumTrack(length: number) {
  ...
  for (let i = 0; i < length; i++) {
    if (Math.random() > 0.6) track.addEvent(new MidiWriter.NoteEvent({ pitch: [drums[0]], duration: "4", velocity: 100 }));
    if (Math.random() > 0.8) track.addEvent(new MidiWriter.NoteEvent({ pitch: [drums[1]], duration: "4", velocity: 90 }));
    if (Math.random() > 0.5) track.addEvent(new MidiWriter.NoteEvent({ pitch: [drums[2]], duration: randomChoice(["8", "16"]), velocity: 80 }));
  }
