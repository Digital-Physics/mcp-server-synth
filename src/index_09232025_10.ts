...
const DRUM_PATTERNS = {
    ...
    verse_A: (track: any) => { track.addEvent([ new MidiWriter.NoteEvent({ pitch: ['C1'], duration: '4', channel: 10, velocity: 100 }), new MidiWriter.NoteEvent({ pitch: ['F#1'], duration: '4', channel: 10, velocity: 70 }), new MidiWriter.NoteEvent({ pitch: ['D1'], duration: '4', channel: 10, velocity: 90 }), new MidiWriter.NoteEvent({ pitch: ['F#1'], duration: '4', channel: 10, velocity: 70 }), ], (event: any, index: number) => ({ startTick: index * 128 })); },
    verse_B: (track: any) => { track.addEvent([ new MidiWriter.NoteEvent({ pitch: ['C1'], duration: '8', channel: 10, velocity: 100 }), new MidiWriter.NoteEvent({ pitch: ['F#1'], duration: '8', channel: 10, velocity: 70 }), new MidiWriter.NoteEvent({ pitch: ['G1'], duration: '8', channel: 10, velocity: 80, wait: '8' }), new MidiWriter.NoteEvent({ pitch: ['F#1'], duration: '4', channel: 10, velocity: 90 }), ], (event: any, index: number) => ({ startTick: index * 64 })); },
    chorus_A: (track: any) => { track.addEvent([ new MidiWriter.NoteEvent({ pitch: ['C1'], duration: '4', channel: 10, velocity: 110 }), new MidiWriter.NoteEvent({ pitch: ['G#1'], duration: '8', channel: 10, velocity: 80 }), new MidiWriter.NoteEvent({ pitch: ['D1'], duration: '4', channel: 10, velocity: 100 }), new MidiWriter.NoteEvent({ pitch: ['G#1'], duration: '8', channel: 10, velocity: 80 }), ], (event: any, index: number) => ({ startTick: index * 128 })); },
    chorus_B: (track: any) => { for(let i=0; i<4; i++) { track.addEvent(new MidiWriter.NoteEvent({ pitch: ['C1'], duration: '8', channel: 10, velocity: 110 })); track.addEvent(new MidiWriter.NoteEvent({ pitch: ['F#1'], duration: '8', channel: 10, velocity: 80 })); } },
    bridge: (track: any) => { track.addEvent(new MidiWriter.NoteEvent({ pitch: ['C1'], duration: '1', channel: 10, velocity: 80 })); },
        ...
        for (let measure = 0; measure < section.length; measure++) {
            ...
            if (isLastMeasureOfSection(measure) && needsFill) {
                ...
                switch(section.name) {
                    case 'verse': pattern = randomChoice([DRUM_PATTERNS.verse_A, DRUM_PATTERNS.verse_B]); break;
                    case 'chorus': pattern = randomChoice([DRUM_PATTERNS.chorus_A, DRUM_PATTERNS.chorus_B]); break;
                    case 'bridge': pattern = DRUM_PATTERNS.bridge; break; // Add this line
                    default: pattern = DRUM_PATTERNS[section.name] || DRUM_PATTERNS.verse_A;
                }
                pattern(track);
            }
...
const ARP_PATTERNS = {
    up: (track: any, chord: string[]) => { for(let j=0; j<4; j++) track.addEvent(new MidiWriter.NoteEvent({ pitch: [chord[j%3]], duration: '16', velocity: 80 })); },
    down: (track: any, chord: string[]) => { for(let j=0; j<4; j++) track.addEvent(new MidiWriter.NoteEvent({ pitch: [chord[(3 - j)%3]], duration: '16', velocity: 80 })); },
    up_down: (track: any, chord: string[]) => { track.addEvent(new MidiWriter.NoteEvent({ pitch: [chord[0]], duration: '16', velocity: 80 })); track.addEvent(new MidiWriter.NoteEvent({ pitch: [chord[1]], duration: '16', velocity: 80 })); track.addEvent(new MidiWriter.NoteEvent({ pitch: [chord[2]], duration: '16', velocity: 80 })); track.addEvent(new MidiWriter.NoteEvent({ pitch: [chord[1]], duration: '16', velocity: 80 })); },
};
...
function generateArpeggioTrack(structure: Section[], chords: { [key: string]: string[][] }, instrument: number): any {
    ...
    track.addEvent(new MidiWriter.ProgramChangeEvent({ instrument }));
    structure.forEach(section => {
        if (section.name !== 'chorus' && section.name !== 'bridge') return;
        const sectionChords = section.name === 'chorus' ? chords.chorus : chords.verse;
        for (let i = 0; i < section.length; i++) {
...
function generateMelodyTrack(structure: Section[], chords: { [key: string]: string[][] }, instrument: number, scale: string[]): any {
    const track = new MidiWriter.Track();
    track.addEvent(new MidiWriter.ProgramChangeEvent({ instrument }));
    structure.forEach(section => {
...
function generateSongMIDI(tone?: string): { midiFile: string; description: string } {
  const structure = generateSongStructure();
  
//   let scale; 
  let scale: string[] = [];
//   let verseProgression;
  let verseProgression: string[] = [];
//   let chorusProgression;
  let chorusProgression: string[] = [];
  ...
  const scaleName = Object.keys(SCALES).find(k => SCALES[k as keyof typeof SCALES] === scale);
...
server.tool(
  ...
  { tone: z.string().optional().describe("Mood or tone (e.g., 'dark', 'jazzy', 'emotional').") },
  async ({ tone }) => {
    try {
      ...
      if (tone === 'emotional') natureVolume = 0.6;
